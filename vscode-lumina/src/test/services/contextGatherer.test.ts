...
import { ContextGatherer, TaskContext, TaskInput } from '../../services/ContextGatherer';
...
suite('ContextGatherer Test Suite', () => {
	...
	test('Should identify relevant patterns via PatternLibrary', async () => {
		...
		// PatternLibrary may not have patterns loaded in test environment, that's ok
		assert.ok(Array.isArray(patterns), 'Should return patterns array');
		assert.ok(patterns.length <= 5, 'Should limit to top 5 patterns');

		// If patterns found, check they're database-related
		if (patterns.length > 0) {
			const hasDbPattern = patterns.some(p => p.id.includes('DB') || p.id.includes('DATABASE'));
			assert.ok(hasDbPattern, 'Patterns should be relevant');
		}
	});
